SIZE_32BIT = 32
MASK_32BIT = 0xFFFFFFFF

# https://devblog.toopher.com/2014/04/25/code-golf-integer-reversal-in-python/
bit_reverse_table_256 = [
  
]

def reverse_32bit_fast(v):
  c = (bit_reverse_table_256[v & 0xff] << 24) | (bit_reverse_table_256[(v >> 8) & 0xff] << 16) | (bit_reverse_table_256[(v >> 16) & 0xff] << 8) | (bit_reverse_table_256[(v >> 24) & 0xff])
  return c

def reverse_32bit_interesting(x):
    x = (((x & 0xAAAAAAAA) >> 1) | ((x & 0x55555555) << 1))
    x = (((x & 0xCCCCCCCC) >> 2) | ((x & 0x33333333) << 2))
    x = (((x & 0xF0F0F0F0) >> 4) | ((x & 0x0F0F0F0F) << 4))
    x = (((x & 0xFF00FF00) >> 8) | ((x & 0x00FF00FF) << 8))
    return ((x >> 16) | (x << 16))

reverse_32bit = reverse_32bit_fast

def invert_32bit(n):
  return ~n & MASK_32BIT

def rol(x, s):
  s %= SIZE_32BIT
  return ((x << s) | (x >> (SIZE_32BIT-s))) & MASK_32BIT

def ror(x, s):
  s %= SIZE_32BIT
  return ((x >> s) | (x << (SIZE_32BIT-s))) & MASK_32BIT
