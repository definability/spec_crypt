//#define EXTRANONCE

#include "paeq-def.h"
#ifndef NO_SUPERCOP
#include "crypto_aead.h"
#endif

#include <stdio.h>
#include "stdint.h"
#include <cstring>
#include <stdlib.h>
#include "string.h"
#include "wmmintrin.h"
#if defined(_MSC_VER)
#include "intrin.h"
#else
#include <x86intrin.h>
#endif
#include "emmintrin.h"

int enc(char* filename, char* key);
int dec(char* filename, char* key);

int main(int argc, char* argv[])
{
    char key[] = "";
    if (*argv[2] == 'd') {
        dec(argv[1], key);
    }
    else {
        enc(argv[1], key);
    }
    /*
	int result = crypto_aead_decrypt(plaintext_decrypted, &decrypted_length, NULL, ciphertext, ciphertext_length, associated_data, ad_length, nonce, key);
    printf("%d: %s\n", result, plaintext_decrypted);
    */
	return 0;
}

int enc(char* filename, char* key) {
    FILE * input = fopen(filename, "rb");
    char plaintext[2048] = {0};
    char s;
    int i=0;
    while (fscanf(input, "%c", &s) != EOF) {
        plaintext[i] = s;
        i++;
    }
    fclose(input);
    unsigned long long plaintext_length = strlen(plaintext);
    unsigned long long ciphertext_length;
    unsigned long long decrypted_length;
    char* plaintext_decrypted = (char*)malloc(strlen(plaintext)+tag_bytes);
    char* ciphertext = (char*)malloc(strlen(plaintext)+tag_bytes);
    char associated_data[] = "ad";
    unsigned long long ad_length = strlen(associated_data);
    char nonce[] = "nonce";
    char * k = (char*) malloc(strlen(key));
    strcpy(k, key);
	crypto_aead_encrypt(ciphertext, &ciphertext_length, plaintext, plaintext_length, associated_data, ad_length, NULL, nonce, k);
    printf("%s ->", plaintext);
    int filename_length = strlen(filename);
    char * enc_filename = (char*) malloc(filename_length+5);
    char * mac_filename = (char*) malloc(filename_length+5);
    strcpy(enc_filename, filename);
    strcpy(mac_filename, filename);
    strcat(enc_filename, ".enc");
    strcat(mac_filename, ".mac");
    FILE * enc_output = fopen(enc_filename, "w");
    FILE * mac_output = fopen(mac_filename, "w");
    for (i=0; i<plaintext_length; i++) {
        printf(" %02X", ciphertext[i] & 0xFF);
        fprintf(enc_output, "%c", ciphertext[i] & 0xFF);
    }
    printf(": ");
//    for (; i<tag_bytes+strlen(plaintext); i++) {
    for (; i<ciphertext_length; i++) {
        printf("%02X ", ciphertext[i] & 0xFF);
        fprintf(mac_output, "%c", ciphertext[i] & 0xFF);
    }
    printf("\n");
    fclose(enc_output);
    fclose(mac_output);
    return 0;
}

int dec(char* filename, char* key) {
    char* plaintext_decrypted = (char*)malloc(2048+tag_bytes);
    char* ciphertext = (char*)malloc(2048);
    unsigned long long decrypted_length;
    char associated_data[] = "ad";
    unsigned long long ad_length = strlen(associated_data);
    char nonce[] = "nonce";
    int filename_length = strlen(filename);
    char * enc_filename = (char*) malloc(filename_length+5);
    char * mac_filename = (char*) malloc(filename_length+5);
    strcpy(enc_filename, filename);
    strcpy(mac_filename, filename);
    strcat(enc_filename, ".enc");
    strcat(mac_filename, ".mac");
    FILE * enc_output = fopen(enc_filename, "rb");
    FILE * mac_output = fopen(mac_filename, "rb");
    int i=0;
    char s;
    printf("->");
    while (fscanf(enc_output, "%c", &s) != EOF) {
        ciphertext[i] = s&0xFF;
        printf(" %02X", s&0xFF);
        i++;
    }
    printf(": ");
    unsigned long long ciphertext_length = (unsigned long long)i;
    while (fscanf(mac_output, "%c", &s) != EOF) {
        ciphertext[i] = s&0xFF;
        printf("%02X ", s&0xFF);
        i++;
    }
    printf("\n");
    ciphertext[i] = 0;
    fclose(enc_output);
    fclose(mac_output);
    ciphertext_length = (unsigned long long)i;

    char * k = (char*) malloc(strlen(key));
    strcpy(k, key);
	int result = crypto_aead_decrypt(plaintext_decrypted, &decrypted_length, NULL, ciphertext, ciphertext_length, associated_data, ad_length, nonce, k);
    printf("%d: %s\n", result, plaintext_decrypted);
    return 0;
}
